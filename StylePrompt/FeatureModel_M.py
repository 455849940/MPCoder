...
class PreferFeatureCodeLlama_M(nn.Module):
    
    def __init__(self, config):
        super(PreferFeatureCodeLlama_M, self).__init__()
    ...
    def forward(self, user_id, input_ids, attention_mask, select_mask, past_key_values = None):
        ...
        #print(select_mask)
        # count_positive = torch.sum(select_mask > 0)
        # print(count_positive.item())
        #print("-----")
        #input()
        ...
        if self.forwardChoose == 1:
            return self.forwardB(user_id, input_emd, new_input_ids, attention_mask, past_key_values, select_mask) #第二阶段MSAAdapter训练加预测
            ...
            return self.forwardsp(user_id,input_emd, new_input_ids, attention_mask, past_key_values) #训练加预测sp+残差的  多个user_id
        elif self.forwardChoose == 3: #draw picture
            return self.forwardTu(user_id, input_emd, new_input_ids, attention_mask, past_key_values, select_mask)
    ...
    def forwardB(self, user_id, w_emd, input_ids, attention_mask, past_key_values = None, select_mask = None):
        ...
        if mask is None:
            ...
            if self.enable_contrast:
                ...
                for i in range(batch_size):
                    # 提取当前数据的 mask 大于 0 位置上的向量
                    selected_vectors = w_emd[i][s_select_mask[i]]
                    #print(selected_vectors.shape)
                    # 计算当前数据的平均向量
                    mean_vector = torch.mean(selected_vectors, dim=0)
                    # 将平均向量添加到列表中
                    mean_vectors_list.append(mean_vector)

                # 将列表转换为张量
               
                s_prompt = torch.stack(mean_vectors_list)
                #print(s_prompt.shape)
                #print(u_prompt.shape)
                #input()
                #s_tensors = torch.index_select(w_emd, dim = 2, index = nonzero_indices)
                #s_prompt = torch.mean(s_tensors,dim=1)
                u_prompt = (u_prompt + s_prompt)/2
                #now
    ...
    def forwardTu(self, user_id, w_emd, input_ids, attention_mask, past_key_values = None, select_mask = None):
        #print("here")
        ...
        if mask is None:
            ...
            Style_time_hidden_states = self.mlp(hidden_states[-1])
            average_sequence_tensors = torch.mean(Style_time_hidden_states, dim=1)
            #print(average_sequence_tensors[0].shape)    
            
            return average_sequence_tensors
            #新推理提供以上两个值，需要下一token的词表分布向量
